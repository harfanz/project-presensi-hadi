...
class DepartemenController extends Controller
{
    ...
    public function store(Request $request){
        ...
        if($simpan){
            ...
            berhasil disimpan']);
             ...
             return Redirect::back()->with(['warning' => 'Data 
            Gagal disimpan']);
        }
    ...
    if($kode_dept_baru != $kode_dept){
        $cek = DB::table('departemen')->where('kode_dept', $kode_dept_baru)->count();
        if($cek > 0){
            return Redirect::back()->with(['warning' => 'Kode departemen '.$kode_dept_baru.' sudah ada']);
        }
    }
